...
#ifdef ARDUINO_NANO_ESP32  // use old ESP32 fork
...
void send_MQTT_LOG() {  // __________________________________________ from ACS diagnostic string if enabled
...
bool is_authenticated() {
  if (Login_Diag) Serial.println("Enter is_authenticated");
  if (server.hasHeader("Cookie")) {
    if (Login_Diag) Serial.print("Found cookie: ");
    String cookie = server.header("Cookie");
    if (Login_Diag) Serial.println(cookie);
    if (cookie.indexOf("THISSESSIONID=1") != -1) {
      if (Login_Diag) Serial.println("Authentication Successful");
      return true;
    }
  }
  if (Login_Diag) Serial.println("Authentication Failed");
  return false;
}
...
void handleLogin() {
  ...
  if (server.hasArg("DISCONNECT")) {
    if (Login_Diag) Serial.println("Disconnection");
    server.sendHeader("Location", "/");
    server.sendHeader("Cache-Control", "no-cache");
    server.sendHeader("Set-Cookie", "THISSESSIONID=0");
    server.send(301); 
    if (Login_Diag) Serial.println("Logout");
    return;
  }
  if (server.hasArg("PASSWORD")) {
    if (server.arg("PASSWORD") == OPERATIONPW ) {
      server.sendHeader("Location", "/");
      server.sendHeader("Cache-Control", "no-cache");
      server.sendHeader("Set-Cookie", "THISSESSIONID=1");
      server.send(301);
      msg = "Login Successful";
      //return;
    } else {
      msg = "Wrong password! try again."; // _________________ send to LOGIN PAGE
    }
    Serial.println(msg);
    BootLog = msg; // ________________________________________ inform REMOTE about LOGIN or LOGIN FAIL
    send_MQTT_LOG();
...
<body><div class=\"login-container\"><form action='/login' method='POST'>\
<div class=\"input-group\"><label for='password'>Password :</label><input type='password' name='PASSWORD' placeholder='password'></div>";
  DYN_HTMLs += "<div class=\"warn\"><b>" + msg + "</b></div></br>";  // msg is about to show if login fails
  DYN_HTMLs += "<div><button type='submit' class=\"btn\" >Login</button></form></div>";
  DYN_HTMLs += "</br><div><p>This site uses Cookies.</br>by using this LOGIN page you accept Cookies being stored on your computer.</p></div>";  // legal
  DYN_HTMLs += "</div></body></html>";
  server.send(200, "text/html", DYN_HTMLs);
}
...
<p>LED state: :";
  ...
  DYN_HTMLs  += "<h2>Fetch New Readings</h2><p>( No Autorefresh )</p>\
<form action=\"./value\">\
<input type=\"submit\" value=\"Fetch values\" />\
</form><hr>";

#ifdef use_ADS1115
  DYN_HTMLs += "<h4>ADS1115 ( differential input ) shows at A0 and A1 as [mV]</h4><hr>";
#endif

#ifdef useACSxxx
#ifdef useACS712
  DYN_HTMLs += "<h4>ACS712 on A0 input [V] now show [+-5A]</h4><p>";
#endif
#ifdef useACS758
  DYN_HTMLs += "<h4>ACS758 on A0 input [V] now show [+-60A]</h4><p>";
#endif
  ...
  DYN_HTMLs  += "<h4>Analog read</h4><h3>";
  DYN_HTMLs  += String(A_Reads); 
  DYN_HTMLs  += "</h3><hr><h4>MQTT pub</h4><h3>";
  DYN_HTMLs  += MQTTs;
  DYN_HTMLs  += "</h3></p><hr>\
  </div>\
\
<div class=\"right\">";
  if (is_authenticated()) {
    DYN_HTMLs += "<a href=\"/login?DISCONNECT=YES\">Logout</a><hr>";
  } else {
    DYN_HTMLs += "<a href=\"/login\">Login</a><hr>";    
  }

  DYN_HTMLs += "<h2>About</h2>\
<p>using <b>";
  DYN_HTMLs  += WIFI_HOSTNAME;
  DYN_HTMLs  += "</b></br>via Arduino IDE</br>+ WEB SERVER</br>+ MQTT (TLS)</br><b>add hardware:</b></br>+ 4 relay board</br>+ INA 219/260</br>+ ACS712/ACS758</br>+ ADS1115</p>\
</div>\
\
</div>\
\
...
// __________________________________________________________________ login with cookie
#ifdef ESP32
  const char *headerkeys[] = {"User-Agent", "Cookie"};
  size_t headerkeyssize = sizeof(headerkeys) / sizeof(char *);
  server.collectHeaders(headerkeys, headerkeyssize);  //ask server to track these headers
#else  // pico w
  server.collectHeaders("User-Agent", "Cookie");   // ask server to track these headers
#endif
