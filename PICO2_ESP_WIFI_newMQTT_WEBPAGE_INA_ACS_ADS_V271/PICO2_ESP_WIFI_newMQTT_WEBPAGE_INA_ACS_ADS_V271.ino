...
String BLOGs = "https://engineering-news.org/kllfusion01/infusions/blog/blog.php?readmore=14";
...
// ACS.ino
String ACSxxxs = "";
boolean ACSxxxuseFilter = true;
boolean ACSxxxdiag = false; //true; // _________________________________ every second print and now also send to mqtt /log


char thisA_Reads[70]; // ________________________________________________ filled by Job 1sec used in HTML
...
// temperature_sensor_config_t temp_sensor = TEMPERATURE_SENSOR_CONFIG_DEFAULT(20, 100); // also not work
...
// or just A0 A1 A2 as predefined 1 2 3 
#endif
...
#ifdef useESP_CAL_VOLT
#if defined(ARDUINO_ESP32S3_DEV)     // that not worked on ESP32S   E (3031) adc_cali: adc_cali_create_scheme_line_fitting(184): default vref didn't set
  uint32_t A0milli = analogReadMilliVolts(A0pin);
  sensorA0val = float(A0milli) /1000.0;
  uint32_t A1milli = analogReadMilliVolts(A1pin);
  sensorA1val = float(A1milli) /1000.0;
  uint32_t A2milli = analogReadMilliVolts(A2pin);
  sensorA2val = float(A2milli) /1000.0;
#endif
#endif
...
#ifdef use4RELAY
  setup_EEPROM(); // _________ see EEPROM.ino to save remote relay setpoint commands ( web-page OR MQTT ) for REBOOT
...
void setup(void) {
  ...
  while (!Serial && (millis() < 10000));  // ________________________ PICO on charger hangs without timeout
  Serial.println();
  ...
  send_MQTT_BootLog(); // ___________________________________________ now after setup want send BootLog to MQTT >> NR copy to REMOTE too
}
...
void loopC () {
  ...
  if ( loopc >= loopclim ) {
    ...
    if (nowsec != thissec) {
      ...
      thismin = ti.tm_min;
      Serial.print(".");
      //Serial.println(thissec); // _________________________ other 1 sec JOBs
      Ains(); // ____________________________________________ also ACS
